                ...
                <div class="card-body">
                    <table class="table" style="font-weight: 600;">
                        ...
                        <tr>
                            <th>Action:</th>
                            <td>
                                @if ($order->return_reason && !$order->return_date)
                                    <form action="{{ route('admin.return.accept', $order->id) }}" method="POST" style="display:inline;" onsubmit="return confirm('Are you sure you want to confirm this return request?');">
                                        @csrf
                                        <button type="submit" class="btn btn-success" data-status="confirm">Accept Return</button>
                                    </form>
                                ...
                                @elseif($order->status == 'delivered')
                                    <button class="btn btn-dark" disabled>Complete Order</button>
                                @elseif($order->status == 'returned')
